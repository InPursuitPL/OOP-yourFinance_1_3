#! python3
# month.py - stores Month class, which holds list of stash objects and
# separate total stash.
...
class Month:
    ...
    def set_name(self):
        """Sets month name (as 3 letter abbreviation only)."""
        while True:
    ...
    def add_stash(self, stashObj):
        """Adds stash to stashes list and adds value to total stash."""
        assert isinstance(stashObj, stash.Stash),\
            'Wrong argument type (should be Stash object).'
        ...
        for obj in self.stashList:
            assert stashObj.name != obj.name,\
                'Stash object with this name already in list.'
    ...
    def remove_stash(self, stashObj):
        """Removes stash and subtracts value from total stash."""
        assert isinstance(stashObj, stash.Stash),\
            'Wrong argument type (should be Stash object).'
        # Function removes Stash objects from the stashList but does it
        # "by name" of the stashes. Please have a look at upper comment
        #  lines in add_stash function to check why I use adding/removing
        #  by name of the objects.
        nothingRemoved = True
        for obj in self.stashList:
    ...
    def show_month(self):
        """Returns string with month name and stashes info."""
